...
- **Commits:** 0f38f80 barpopout: split into per-key modules, share a popout shell; a follow-up commit folding notifications/clipboard into BarPopout itself (merged into dev via merge commits)
- **Original TODO:** none — a direct request in this session, not a `docs/TODO.md` entry.

### What was asked
Clean up `Services/BarPopout.qml`: replace its accumulated inline commentary with minimal, atomic documentation, keeping every behavior exactly as it was. Separate `Components/BarPopout/BarPopout.qml` — the single 2106-line file holding all 14 of the popout's "which"-keyed sections (volume, brightness, wifi, ethernet, bluetooth, network, timer, stopwatch, battery, microphone, camera, power, status, stats) inline in one `Component` — into well-organised, per-context files, following this repo's own conventions. Then reintegrate Calendar, Clipboard and Notifications, which had gone dark. A first pass rebuilt Calendar and gave Clipboard/Notifications their own standalone overlay windows (still built on a new shared popout shell); a follow-up request in the same session asked for notifications/clipboard specifically to be folded all the way into BarPopout itself instead — two more `which` cards, not separate windows — with every feature, including the Super+N/Super+Shift+V keybinds, preserved.

### What was done
**`Services/BarPopout.qml`:** every accumulated planning-journal comment condensed to short, atomic notes (no behavior change), then extended to also own the "notifications"/"clipboard" keys: `notificationsIconRightX`/`clipboardIconRightX` (registered icon-position getters, migrated verbatim from the now-deleted `Services/NotificationPanel.qml`) and `openNotifications()`/`toggleNotifications()`/`openClipboard()`/`toggleClipboard()`, each resolving to the same `open(key, x)`/`hide()` every other key already uses.
...
- `Components/BarPopout/modules/{Volume,Brightness,Wifi,Ethernet,Bluetooth,Network,Timer,Stopwatch,Battery,Microphone,Camera,Power,Status,Stats,Notifications,Clipboard}.qml` — one file per "which" key, each taking a plain `active` bool (what a `root.which === "x"` check used to gate) and, where needed, `chWidth`. Every section stays instantiated for the shell's whole session, same as before — never `Loader`-swapped — so per-section state (`Status.qml`'s tiling-mode highlight, live countdown timers, Notifications' collapsed-group state) survives close/reopen exactly as it did in the monolith; a `Loader`-per-key design was considered and rejected after finding it would silently drop `Widgets.StaggerReveal`'s reveal-cascade animation on every reopen, not just the first one.
- `Components/BarPopout/modules/Header.qml` — the shared title row + settings icon + separator, one component instead of duplicated per-section markup. Notifications/Clipboard both return `""` from `title()` and keep their own inline header (a DND row, a search bar) instead.
...
- `Widgets/Format.js` — the `rate()`/`countdown()`/`stopwatch()` formatters shared across Wifi/Network/Timer/Stopwatch/Stats (and Calendar's own timer list), pulled out as a pure `.pragma library` file, same convention as `Bar/glyphs.js`.
- `Components/BarPopout/BarPopout.qml` itself: window-level state (`which`, `chWidth`, the corner-radius/anchor math, the net/stats watch-gating, the `power`/`notifications`/`sidebar` IPC targets) plus one `Column` instantiating all 16 modules and the header. Notifications/Clipboard are the two size exceptions — `cardWidth`/`_wideCardAvailableHeight` special-case them to `min(32% of screen, 46ch)` wide and up to 3/4 screen tall, instead of the standard chWidth formula every other key uses; their corner radii need no special-casing, the existing nearest-corner rule already produces the right answer for a top-bar, right-isle key.

**A shared popout shell, `Widgets/PopoutSurface.qml`:** every corner overlay in this shell was independently reimplementing the same ~140 lines — the full-screen transparent `PanelWindow`, the `exclusiveZone: -1` double-count fix, the fade-in `Item`, the click-outside-to-dismiss `MouseArea`, `Services.LayerFocus`, the anchored/clamped card position, and `Widgets.Panel`'s per-corner radii. Factored into one reusable component, used today by `Components/BarPopout/BarPopout.qml` and `Components/Calendar/Calendar.qml` (`anchorEdge: "center"`, its own one case — the only overlay that centres under its trigger, the bar clock, instead of hugging an edge).

**Calendar, Clipboard and Notifications had gone fully dark**, not just uncleaned: Calendar's `.qml` sat in `_TRASH/Panels/`, and `Services/NotificationPanel.qml`'s own two overlay windows were disconnected the same way — `Services/Calendar.qml`'s own `shown` singleton was live and the bar clock already called `toggle()` on it, but nothing was left to actually show. (`docs/VERIFICATION.md`'s own prior entry, "phi-shell comments read like a development log", flagged this exact gap as finding 1 without acting on it — "not a decision that cleanup pass should make unilaterally.")
- `Components/Calendar/Calendar.qml` — rebuilt on `Widgets/PopoutSurface.qml`: the flip-clock readout inline, plus two new modules, `modules/MonthGrid.qml` (the interactive read-only month grid) and `modules/TimerList.qml` (the timer creation control + list, reusing `Widgets/Format.js`). Registered in `Components/qmldir`, mounted in `shell.qml`.
- Notifications and clipboard instead became two more `Components/BarPopout/modules/` cards (`Notifications.qml`, `Clipboard.qml` — content migrated verbatim from the now-deleted `Services/NotificationPanel.qml` and its two overlay windows, only the sizing/lifecycle wiring and four `Services.NotificationPanel.hide()`/`Keys.onEscapePressed` call sites changed to `Services.BarPopout.hide()`). `Services/NotificationPanel.qml` is deleted outright — its state (`notificationsShown`/`clipboardShown`/anchors), its mutual-exclusion `Connections`, and its two `IpcHandler`s (targets `notifications`, `sidebar`) all moved onto `Services/BarPopout.qml` and `Components/BarPopout/BarPopout.qml`. Every peer surface that used to call `Services.NotificationPanel.hide()` as part of its own mutual-exclusion `onShownChanged` (`Services/AgentPanel.qml`, `Services/Calendar.qml`, `Services/ConfirmDialog.qml`, `Services/SettingsPanel.qml`) had that now-dead line removed — `Services.BarPopout.hide()`, already present in every one of them, covers notifications/clipboard automatically now that they are `which` keys. The bar's bell (`Components/Bar/modules/Notifications.qml`) and clipboard (`Components/Bar/modules/Clipboard.qml`) icons, and `Components/Toast.qml`'s click-to-open, all repointed from `Services.NotificationPanel.*` to the identically-shaped `Services.BarPopout.*`.

Checked by hand against the pre-edit content (a `diff` against each file's last-committed version, not just a read-through) rather than by the `/code-review` skill, which hit this session's spend limit on the second pass; the diff confirmed the only non-comment changes in the two migrated modules are exactly the ones described above — no other line drifted.

### Honest assessment
This shell cannot be run in this environment (no compositor, per `phi-shell/CLAUDE.md`) and this environment's own QML language server could not resolve even `import QtQuick` for any file touched — it was checked by hand against the deleted originals, not by a linter or the shell itself. Nothing here is hardware-verified.
...
4. Click the bar's notification bell — a card should drop below it (wider/taller than the others) with the DND row, quick-triggers, and the notification list grouped by date then source; click the bar's clipboard icon — a similarly wide card with an auto-focused search field, pinned entries, then history. Both used to be their own separate windows anchored under their icon; confirm they now behave identically but as BarPopout cards (opening one closes any other BarPopout card, Calendar, the agent panel or Settings, same as before).
5. Keybind check: `Super+N` opens/toggles the notifications card, `Super+Shift+V` opens/toggles the clipboard card, both anchored under their own bar icon exactly as a click would. In the notifications card, typing in the clipboard's search field, pressing arrows/Tab to move the selection, Enter to restore-and-close, Escape to close, and Ctrl+P to toggle-pin the selected entry should all still work.
6. Confirm mutual exclusion still holds: with any one overlay open (a BarPopout card — including notifications/clipboard now — Calendar, the AI agent panel, or Settings), opening any other one closes the first.
...
- **Repo / branch:** phi-shell / dev, phi / dev, phios-dotfiles / dev
- **Commits:**
  - `phi-shell`: `cb218e2` widgets: add asymmetric corner-radius primitive, wire into Panel · `cc415d6` widgets: shade-based surface colours for Panel/buttons/Segment · `27ac4b0` widgets: thin the toggle track border to the hairline token · `a33a4a5` widgets: add StaggerReveal, a per-child cascade-fade primitive · `0ad7939` bar: split the single status bar into a top and bottom pair · `7b098b6` bar: add a separator module for the isle line dividers · `305a18b` widgets: round bar isles per-corner, thin outward and thick inward · `88e173b` bar: restyle workspace squares and drop per-program pinning · `4bd2e58` bar: add the six new bar modules the rework layout needs · `280677b` bar: drop the remaining value text labels, icon only · `eb95b73` panels: split notifications and clipboard into independent overlays · `8dc30bc` panels: add a timer and an interactive month calendar to the overlay · `cdcf223` barpopout: anchor bottom-bar overlays correctly, expand network/bt/sound · `61eb111` barpopout: add the status and stats overlays · `ae5d943` docs: fix ADR 078 registry list after tabs.json retirement · `4e197cd` agentpanel: contain dock to both bars, rename status tab, relocate settings · `7dd7594` agent status tab: add system-status overview above the proposals list · `0763287` chat: retarget the pending-proposals cue to the renamed status section · `be8cac9` chatshell: make the projects/chat-list sidebar toggleable · `d1f3d0c` lock: remove recent-notifications list from the lock screen · `06e6de2` alttab: show only the viewed workspace, pan without closing on a pill click · `cd5156c` background: wire empty-desktop right-click context menu · `cddfc88` images: add native floating image window surface · `3e94773` config: split colour off Tokens.qml onto a watched Colors.json · `f497d62` background: crossfade the wallpaper base colour on a theme switch · `8feaa5f` shell: store the theme schedule in phi state, not a private file · `0bc9c3e` docs: split the Tokens/Colors worked example to match the real shape · `9f9181d` brightness: drop the sun/moon eclipse icon, add a plain BrightnessIcon · `3e1b39f` merge into dev
  - `phi`: `f0439e8` state: add theme.schedule for the auto light/dark theme option · `74f7c9e` query: point image opens at phi-shell's native image window · `3af4ad4` merge into dev
  - `phios-dotfiles`: `2d1d4d9` desktop: add thunar GUI file manager · `94c71cc` hypr: drop the dedicated btop/Steam workspace pinning · `85b2390` hypr: document the workspace-switch loop as a real open gap · `5bf1f56` quickshell: split colour tokens out of Tokens.qml into Colors.json · `3a2991d` hypr+yazi: point image opens at phi-shell's native image window · `ead3802` design: raise terminal padding to 20px per rework.md s2 · `c668453` merge into dev
- **Original TODO:** the full text of `rework.md` (workspace root) — claimed, worked, and (per the *Partial completion* rule) removed from `docs/TODO.md` in full; the genuine remainders are re-added below as their own bare entries.
- **Requires phi rebuild:** not yet tagged — `phi`'s changes are only on `dev` (the new `theme.schedule*` state keys and the image-open IPC command); a tag should be cut once this is promoted to `main`, following this repo's own convention of releasing from there.

### What was asked

Implement everything described in `rework.md`: a two-bar (top+bottom) status bar layout with a different module set on each; a full rework of every status-bar overlay (calendar, a new "status" overlay, notifications, clipboard, a new "stats" overlay, network, bluetooth, sound, screen); a restructured AI chat panel (chat/code-session/status tabs); several removed features (per-program workspace pinning, the notification/clipboard side panel, icon text labels, the brightness icon's sun/moon fill); several changed/fixed behaviours (workspace-switch looping, Thunar as a GUI file manager, the image-panel focus bug, an "auto" light/dark theme option that doesn't reset the shell session); a lock-screen and window-overview rework; a handful of new UI elements (a system context menu, a custom image window); and five cross-cutting stylistic notes (s1–s5: thinner borders, shade-based colour instead of black/white contrast, compound nested transitions, 20px terminal padding, and an overall more elegant terminal-coherent style).

### What was done

Landed in six phases on `phi-shell`'s `dev` (via a local `rework/interface` branch), plus small companion changes in `phi` and `phios-dotfiles`:

1. **Style foundation** — `Widgets/AsymmetricPanel.qml` (a Canvas-drawn per-corner radius primitive, since no Qt-version guarantee exists for `Rectangle`'s native per-corner properties), wired into `Widgets/Panel.qml`; a new `ambient: "shaded"` recipe in `Widgets/WidgetStates.js` so `Panel`/`SmallButton`/`StyledButton`/`Segment` read shade ramps (`bg-1/2/3`, `border`/`borderStrong`) instead of full black/white inversion; `Toggle`/`Segment`/`StyledButton` moved to the hairline border token; `Widgets/StaggerReveal.qml`, a new per-child cascade-fade primitive, used throughout the later phases' overlay content.
2. **Bar restructure** — `Bar/Bar.qml` parameterized by a new `edge` property (top/bottom) rather than duplicated; `Bar/modules-top.json`/`Bar/modules-bottom.json` replace the single `Bar/modules.json`; six new bar modules (`Separator`, `Lens`, `CurrentApp`, `WindowList`, `Stats`, `StatusMenu`, `NetworkStatus` — the last merging the old separate wifi/ethernet/tailscale icons into one); `Bar/modules/Workspaces.qml` restyled to plain numbered squares with no per-program pinning; every remaining bar-icon text label removed; `Widgets/BrightnessIcon.qml` replaces the sun/moon eclipse icon with a plain brightness glyph that keeps only the fill animation.
3. **Overlay rework** — `Panels/Sidebar.qml`/`Panels/tabs.json` retired; `Panels/NotificationsOverlay.qml`/`Panels/ClipboardOverlay.qml` are their own overlays now (notifications gained date→app grouping and 30m/1h/4h DND quick-triggers); `Panels/Calendar.qml` gained a timer and a read-only interactive month grid; new "status" (profile, power actions, real MPRIS media controls, night-mode/True-Tone/stay-awake/microphone toggles, a tiling-mode grid) and "stats" (network/disk/RAM/CPU/GPU stats, fan-profile buttons) sections in `Panels/BarPopout.qml`; the network overlay folded in the old standalone wifi/ethernet sections plus a real Wi-Fi radio switch and Tailscale connect switch; bluetooth/sound overlays gained real device lists.
4. **AI agent panel** — `Panels/AgentPanel.qml`'s three tabs are now chat/code-session/**status** (was chat/code/memory — the renamed tab gained a system-health summary above the existing memory-proposals list); the dock and its scrim now stop clear of the bottom bar too, not just the top; the settings button moved to the panel's own top-right corner; the chat sidebar (projects/chat list) is now collapsible.
5. **Lock screen + overview** — the lock screen's recent-notifications list is removed (reboot/shutdown confirmation was already correctly wired, verified not rebuilt); `AltTab/AltTab.qml` now shows only the currently-viewed workspace's windows, centred, and a workspace-pill click pans the view (cross-fade) instead of switching Hyprland's real workspace and closing.
6. **Remaining features** — a right-click context menu on the empty desktop (`Background/Background.qml`, reusing the previously-unwired `Widgets/ContextMenu.qml`); a native floating image window (`Images/ImageWindow.qml`, a real Wayland `FloatingWindow` with a native `startSystemMove()` drag and a native fullscreen toggle, not a `PanelWindow` simulation) replacing the external `imv` + Hyprland-window-rule approach, with `phi`'s launcher and yazi's opener repointed at it via a new `qs ipc call image open <path>` target; Thunar added to `phios-dotfiles`; and the live auto-theme mechanism — `phios-dotfiles`' `Config/Tokens.qml.tmpl` split into structural tokens (unchanged) plus a new `Config/Colors.json.tmpl` (everything that actually varies by variant), consumed by a new `Config/Colors.qml` singleton whose own source never changes (only the JSON it watches does) — this is the actual fix, verified end to end with a real `phi theme set --dry-run`/`phi theme render` against this checkout. `Services/ThemeSchedule.qml` and its Settings UI (Off/Automatic/Custom-hours) turned out to already exist from an earlier, separate pass with its own still-open `docs/VERIFICATION.md` entry below this one — this rework migrated its storage from a private JSON file to the new `phi state` keys and left its scheduling logic otherwise untouched.

### Honest assessment

<span style="color:red">**NOT DONE:**</span> a handful of pieces `rework.md` asked for have no real, working backend and ship as honest, inert UI instead of a fake one — each is re-added to `docs/TODO.md` below as its own open task:
- The workspace-switch/gesture loop (Super+Ctrl+arrows/h/l, the three-finger swipe) still wraps — Hyprland's own relative selector has no non-wrapping form, and a real fix needs a Hyprland Lua workspace-enumeration API this project has never confirmed live. Left undiagnosed rather than risk breaking a keybind that currently works.
- The stats overlay's four fan-profile buttons are real UI with no backend — no fan-control mechanism exists anywhere in this codebase or via any official-repo package.
- The status overlay's tiling-mode grid: only "Tile" and "Floating" map to a real Hyprland dispatch; X scroll, Y scroll, Center and Fair have no stock-Hyprland equivalent (only third-party plugins, outside rule 2).
- The status overlay's camera-sensor toggle is a confirmed no-op (no v4l2 precedent anywhere in this codebase, and a real permission system is a separate, already-tracked, not-yet-built feature).
- s2's "terminal padding area should be draggable when floating" — the padding itself is now 20px, but kitty has no config option to treat a click in its own padding as a window-move request, and Hyprland has no per-region mouse binding to fake it from the compositor side.
- The network overlay's Wi-Fi section has no real speedtest trigger, only the passive live-rate graph.

Everything else is reasoned complete by reading the code, not by running it — **this whole environment has no compositor** (`phi-shell/CLAUDE.md`: "You cannot run this"), so nothing in any of the six phases has been visually confirmed. Specific things a screenshot pass should look at first: the outward/inward bar-corner interpretation (rework.md doesn't define which corner is "outward," so this is a judgment call, consistently applied); `HyprlandToplevel.workspace`/`.wmClass` (used by the new `WindowList` bar module) are inferred from this project's own established sibling-property naming, not confirmed against a live session; the `phios-image` window-title-matching Hyprland rule for the new image window is unconfirmed on real hardware; and whether the crossfade transition on a theme switch is visually smooth rather than just structurally wired.

Two things intersect with already-open items elsewhere: the `Services/ThemeSchedule.qml` migration touches a feature with its own still-open `docs/VERIFICATION.md` entry (below) awaiting separate sign-off, and the network-overlay expansion substantially (but not completely — no compressed/expanded two-state toggle, no active-servers-with-killswitches list) overlaps an existing `docs/TODO.md` "Older" bug entry on merging the network/VPN/Tailscale bar icon and overlay, which is left open rather than marked done.

### How to test it

This needs a real Hyprland session on `razer` or `zotac` — nothing here can be exercised from this environment.

1. `cd ~/.local/share/phios/dotfiles && git pull` (or wherever `$PHI_DOTFILES` resolves), then rebuild and reinstall `phi` from this `dev` (or promote to `main` and tag first, per the pending-rebuild note above) and reinstall `phi-shell` from its `dev` the same way. Run `phi theme set dark` once to regenerate `~/.config/quickshell/phi/Config/{Tokens.qml,Colors.json}` — confirm both files exist and `Colors.json` is valid JSON.
2. Look at the top and bottom bars: Φ icon + workspaces + a thin vertical separator on the top-left, clock centred, clipboard/notifications/a settings-glyph icon on the top-right; a runner-bar icon + current app name on the bottom-left, the current workspace's window icons centred, brightness/volume/network/bluetooth/battery/stats icons on the bottom-right. No icon should have text next to it except the workspace numbers and the current-app-name/clock text. Workspace squares should show a thin border at rest and a wider, fully-inverted look when active.
3. Click each right-isle icon on both bars and confirm its overlay opens directly under it, corners rounded thin on the corner nearest the icon and thicker on the other three (both top corners thin for the calendar, opened from the clock).
4. Open the calendar (click the clock): confirm the flip-clock header, a timer section, and a clickable month grid with today highlighted.
5. Click the notifications icon and the clipboard icon separately — each should open its own small overlay (not a shared tabbed panel), notifications grouped by date then by app with 30m/1h/4h DND buttons, clipboard with its search bar, pinned section, and hover-preview.
6. Open the status overlay (top-right "settings" icon) and the stats overlay (bottom-right icon) and look at every section listed in the "What was done" §3 above; confirm the fan-profile buttons and four of the six tiling-mode buttons visibly indicate they don't do anything real yet (per the Honest assessment above) rather than silently failing.
7. Open the AI agent panel (the Φ bar icon or Super+P): confirm three tabs (chat/code session/status), the settings gear in the panel's own top-right corner, the chat sidebar collapsing via its own toggle, and that the panel's dim and dock both stop above the bottom bar, not overlapping it.
8. Lock the screen (Super+L or the status overlay's lock button): confirm no notification list appears, and that reboot/shutdown from the lock screen's power row ask for confirmation before doing anything.
9. Open the window overview (Alt+Tab or the three-finger gesture): confirm only the current workspace's windows show, centred; click a different workspace pill in the bottom strip and confirm the view pans (a brief fade) without the overview closing; then Alt+Tab-select or click a window and confirm it both focuses and closes the overview.
10. Right-click an empty area of the desktop (no window under the cursor) and confirm a menu with run/terminal/files/browser/settings appears and each entry does the right thing.
11. Open an image (from the launcher's file search or Thunar) and confirm it opens in phi-shell's own floating window: a visible border, a bottom strip showing the filename that can be dragged to move the window, and double-clicking the image toggles fullscreen.
12. In Settings › Theme, switch the schedule to "Automatic" (or pick System/Auto if that's the label used) and confirm the shell's colours crossfade smoothly on the next scheduled switch without any panel losing its open state, scroll position, or in-progress input.
