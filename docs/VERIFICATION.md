...
- **Commits:** phi-shell 58fd585 "bar: add a power icon + overlay, gate reboot/shutdown behind confirm"; phi 43142e6 "query: add hibernate/reboot/shutdown to the launcher's system actions"; phi-shell 1a290ac "launcher: fix power confirm dialog keyboard handling" (amendment, same date — see below)
...
**Amendment, same day, before sign-off:** a second-opinion review caught a real keyboard bug in the runner bar's confirm sub-view (`Launcher/Launcher.qml`), fixed in `phi-shell` 1a290ac. `Widgets.StyledButton` has no keyboard handling of its own (`TapHandler` only) — the confirm Column's own `Keys.onReturnPressed` unconditionally ran `Services.PowerActions.perform(...)`, so tabbing to the "Cancel" button and pressing Return still performed the destructive action instead of cancelling; Escape's reliability depended on QML key-propagation details that were not verifiable without a running compositor. Fixed by giving both the confirm and Cancel buttons their own explicit `Keys.onReturnPressed`/`Keys.onEscapePressed`, so keyboard activation is tied directly to whichever button actually has focus rather than relying on the container catching an unhandled event. This only affects the runner-bar confirm sub-view — the bar-icon popout's confirm section (`Panels/BarPopout.qml`) has no keyboard handling at all, before or after this fix; it is mouse/tap-only, a separate, pre-existing gap this entry does not attempt to close.
...
6. Open the runner bar (the launcher) and type "lock" / "suspend" / "hibernate" / "log out" / "reboot" / "shut down". Expected: each appears as a result under "System action". Selecting Lock/Suspend/Hibernate/Log out runs immediately. Selecting Reboot or Shut down instead shows a confirm sub-view ("Reboot now? This cannot be undone." with a Reboot/Cancel button pair) — Enter (with no button tabbed to) or clicking the action button confirms; Escape or clicking Cancel backs out to the search field. Specifically re-check the bug this entry's amendment fixed: press Tab until the "Cancel" button is focused, then press Return — expected result is that it cancels (returns to the search field), NOT that it reboots/shuts down.
