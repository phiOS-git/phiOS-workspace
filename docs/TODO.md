...
- rework status bar buttons: they should not have a box button but be just icons, with hover and active states. Apply SVG animations to icon when changing within states (eg. volume amount and muted, bluetooth activation, wifi strenght/activation/searching, brightness amount (sun/moon icon that fills up, based on either night mode on or not, with an animation from sun to moon)), notifications (DND state as well), battery states. Apply the background to the isles in the status bar (noo trasparency). — [taken: the brightness sun/moon eclipse-transition icon clause only] 2026-09-11, the rest of this bundled entry (box-button rework, volume/bluetooth/wifi/notifications/battery animated icons, isle backgrounds) is still open.
