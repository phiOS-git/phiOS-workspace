...
| `phi-shell` — panels, launcher, lock, overview, screenshot, notifications, settings, magnifier, OSD | **Built and in use**, refined over four restyle rounds + a settings overhaul + a two-round 2026-09-14 expert UI/UX pass (cursor/hover affordance system-wide including drag surfaces, a shared tab-vs-button grammar, search clear buttons, a Settings "Advanced" toggle, a rebuilt clipboard tab, a lock-screen lockout, a two-intensity scrim, a dead bar module reconnected to its real backend, several fully-built-but-never-wired capabilities — chat pin, chat rename, AI Agent config edit/restart — actually wired up, free-text "type a sound name" fields replaced with real pickers — see `docs/VERIFICATION.md`), but the same 2026-09-11 verification round found Alt-Tab "completely broken" (no focus, no close-on-release, no workspace change), the magnifier still doesn't zoom, and screenshot/OCR/QR area-selection offset is still wrong — tracked in `docs/TODO.md`. Neither UI/UX-pass round is hardware-verified yet (no compositor in either session) |
